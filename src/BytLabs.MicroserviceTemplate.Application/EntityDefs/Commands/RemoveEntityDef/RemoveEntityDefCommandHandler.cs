using AutoMapper;
using BytLabs.Application.CQS.Commands;
using BytLabs.Application.DataAccess;
using BytLabs.MicroserviceTemplate.Application.EntityDefs.Dtos;
using BytLabs.MicroserviceTemplate.Domain.EntityDefs;

namespace BytLabs.MicroserviceTemplate.Application.EntityDefs.Commands.RemoveEntityDef
{
    public class RemoveEntityDefCommandHandler : ICommandHandler<RemoveEntityDefCommand, EntityDefDto>
    {
        private readonly IRepository<EntityDef, Guid> repository;
        private readonly IMapper mapper;

        public RemoveEntityDefCommandHandler(IRepository<EntityDef, Guid> repository, IMapper mapper)
        {
            this.repository = repository;
            this.mapper = mapper;
        }

        public async Task<EntityDefDto> Handle(RemoveEntityDefCommand request, CancellationToken cancellationToken)
        {
            var def = await repository.GetByIdAsync(request.Id, cancellationToken);
            def.Remove(); // soft delete
            var result = await repository.UpdateAsync(def, cancellationToken);
            return mapper.Map<EntityDefDto>(result);
        }
    }
}
